import { controller } from "../app/template/template.controller";
...
describe("Template Controller", () => {
	...
	beforeEach(() => {
		prisma = {
			template: {
				...
				findFirst: async (params: Prisma.TemplateFindFirstArgs) =>
					params.where?.id === mockTemplate.id ? mockTemplate : null,
				findUnique: async (params: Prisma.TemplateFindUniqueArgs) =>
					params.where?.id === mockTemplate.id ? mockTemplate : null,
				create: async (params: Prisma.TemplateCreateArgs) => ({
					...mockTemplate,
					...params.data,
				}),
				update: async (params: Prisma.TemplateUpdateArgs) => ({
					...mockTemplate,
					...params.data,
				}),
				delete: async (params: Prisma.TemplateDeleteArgs) => ({
					...mockTemplate,
					id: params.where.id,
				}),
			},
		...
		templateController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		...
		it("should group templates by type field", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".getById()", () => {
		it("should return a template", async function () {
			...
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockTemplate.id });
		});
		...
		it("should handle non-existent template", async function () {
			this.timeout(TEST_TIMEOUT);
	...
	describe(".create()", () => {
		it("should create a new template", async function () {
			...
			const createData = {
				...
				description: "Template for contact forms with validation",
			};
			req.body = createData;
		...
		it("should create a new template with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Template",
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Template",
				description: "Template from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Template",
				description: "Template that will cause Prisma error",
			};
			req.body = createData;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Template",
				description: "Template that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update template details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Template",
				description: "Updated template with additional validation and styling options",
			};
			req.params = { id: mockTemplate.id };
			req.body = updateData;
		...
		it("should handle non-existent template update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Template",
			};
	...
	describe(".remove()", () => {
		...
		it("should handle non-existent template deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await templateController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle special characters in template data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Template 1", type: "email", category: "marketing" },
		{ id: 2, name: "Template 2", type: "sms", category: "notification" },
		{ id: 3, name: "Template 3", type: "email", category: "marketing" },
		{ id: 4, name: "Template 4", type: null, category: "general" },
		{ id: 5, name: "Template 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Template 1");
			expect(result).to.have.property("Template 2");
			expect(result).to.have.property("Template 3");
			expect(result).to.have.property("Template 4");
			expect(result).to.have.property("Template 5");
			expect(result["Template 1"]).to.have.length(1);
		});
	});
});
