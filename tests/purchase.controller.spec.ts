import { controller } from "../app/purchase/purchase.controller";
...
describe("Purchase Controller", () => {
	...
	const mockPurchase = {
		...
		name: "User Registration Purchase",
		description: "Purchase for user registration forms",
		type: "email",
		createdAt: new Date(),
		updatedAt: new Date(),
	};
	...
	const mockPurchases = [
		...
		},
		{
			...
			name: "SMS Notification Purchase",
			description: "Purchase for SMS notifications",
			type: "sms",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			name: "Email Marketing Purchase",
			description: "Purchase for email marketing campaigns",
			type: "email",
			createdAt: new Date(),
			updatedAt: new Date(),
		},
		{
			...
			description: "Purchase without type",
			type: null,
			createdAt: new Date(),
			updatedAt: new Date(),
		},
	...
	beforeEach(() => {
		prisma = {
			purchase: {
				findMany: async (_params: Prisma.PurchaseFindManyArgs) => {
					// Return multiple purchases for grouping tests
					if (req.query?.groupBy) {
						return mockPurchases;
					}
					return [mockPurchase];
				},
				count: async (_params: Prisma.PurchaseCountArgs) => {
					// Return count based on whether grouping is requested
					if (req.query?.groupBy) {
						return mockPurchases.length;
					}
					return 1;
				},
				findFirst: async (params: Prisma.PurchaseFindFirstArgs) =>
					params.where?.id === mockPurchase.id ? mockPurchase : null,
				findUnique: async (params: Prisma.PurchaseFindUniqueArgs) =>
					params.where?.id === mockPurchase.id ? mockPurchase : null,
				create: async (params: Prisma.PurchaseCreateArgs) => ({
					...mockPurchase,
					...params.data,
				}),
				update: async (params: Prisma.PurchaseUpdateArgs) => ({
					...mockPurchase,
					...params.data,
				}),
				delete: async (params: Prisma.PurchaseDeleteArgs) => ({
					...mockPurchase,
					id: params.where.id,
				}),
			},
		...
		purchaseController = controller(prisma as PrismaClient);
		sentData = undefined;
		statusCode = 200;
		req = {
			query: {},
	...
	describe(".getAll()", () => {
		it("should return paginated purchases", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { page: "1", limit: "10" };
			await purchaseController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
		});

		it("should group purchases by type field", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should group purchases by name field", async function () {
			this.timeout(TEST_TIMEOUT);
			...
			expect(sentData.data.grouped).to.have.property("User Registration Purchase");
			expect(sentData.data.grouped).to.have.property("SMS Notification Purchase");
		});

		it("should handle purchases with null values in grouping field", async function () {
			this.timeout(TEST_TIMEOUT);
			req.query = { groupBy: "type" };
			await purchaseController.getAll(req as Request, res, next);
			expect(statusCode).to.equal(200);
	...
	describe(".getById()", () => {
		it("should return a purchase", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPurchase.id };
			await purchaseController.getById(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.deep.include({ id: mockPurchase.id });
		});
		...
		it("should handle non-existent purchase", async function () {
			this.timeout(TEST_TIMEOUT);
		...
		it("should handle Prisma errors", async function () {
			...
			// Mock Prisma to throw an error
			prisma.purchase.findUnique = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
	...
	describe(".create()", () => {
		it("should create a new purchase", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Contact Form Purchase",
				description: "Purchase for contact forms with validation",
			};
			req.body = createData;
			await purchaseController.create(req as Request, res, next);
		...
		it("should create a new purchase with type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Email Purchase",
				description: "Purchase for email notifications",
				type: "email",
			};
			req.body = createData;
			await purchaseController.create(req as Request, res, next);
		...
		it("should create a new purchase without type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Generic Purchase",
				description: "Purchase without type",
		...
		it("should handle form data (multipart/form-data)", async function () {
			...
			const createData = {
				name: "Form Purchase",
				description: "Purchase from form data",
				type: "form",
			};
			req.body = createData;
		...
		it("should handle form data (application/x-www-form-urlencoded)", async function () {
			...
			const createData = {
				name: "URL Purchase",
				description: "Purchase from URL encoded data",
			};
			req.body = createData;
			(req as any).get = (header: string) => {
				if (header === "Content-Type") {
		...
		it("should handle validation errors", async function () {
			...
			const createData = {
				name: "",
				description: "Purchase with empty name",
			};
			req.body = createData;
			await purchaseController.create(req as Request, res, next);
			expect(statusCode).to.equal(400);
		...
		it("should handle Prisma errors", async function () {
			...
			const createData = {
				name: "Test Purchase",
				description: "Purchase that will cause Prisma error",
			};
			req.body = createData;

			// Mock Prisma to throw an error
			prisma.purchase.create = async () => {
				const error = new Error("Database connection failed") as any;
				error.name = "PrismaClientKnownRequestError";
				error.code = "P1001";
				throw error;
		...
		it("should handle internal errors", async function () {
			...
			const createData = {
				name: "Test Purchase",
				description: "Purchase that will cause internal error",
			};
			req.body = createData;
	...
	describe(".update()", () => {
		it("should update purchase details", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Enhanced Contact Form Purchase",
				description: "Updated purchase with additional validation and styling options",
			};
			req.params = { id: mockPurchase.id };
			req.body = updateData;
			await purchaseController.update(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
			expect(sentData).to.have.property("data");
			expect(sentData.data).to.have.property("id");
		...
		it("should update purchase type field", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				type: "sms",
			};
		...
		it("should update multiple purchase fields including type", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Email Purchase",
		...
		it("should handle non-existent purchase update", async function () {
			this.timeout(TEST_TIMEOUT);
			const updateData = {
				name: "Updated Purchase",
			};
	...
	describe(".remove()", () => {
		it("should delete a purchase", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: mockPurchase.id };
			await purchaseController.remove(req as Request, res, next);
			expect(statusCode).to.equal(200);
			expect(sentData).to.have.property("status", "success");
		});
		...
		it("should handle non-existent purchase deletion", async function () {
			this.timeout(TEST_TIMEOUT);
			req.params = { id: "507f1f77bcf86cd799439099" };
			await purchaseController.remove(req as Request, res, next);
			expect(statusCode).to.equal(404);
	...
	describe("Edge Cases and Integration", () => {
		...
		it("should handle very long purchase name", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "A".repeat(1000), // Very long name
				description: "Purchase with very long name",
			};
			req.body = createData;
			await purchaseController.create(req as Request, res, next);
			expect(statusCode).to.equal(201);
		...
		it("should handle special characters in purchase data", async function () {
			this.timeout(TEST_TIMEOUT);
			const createData = {
				name: "Purchase with special chars: !@#$%^&*()",
		...
		it("should handle concurrent requests", async function () {
			...
			const createData = {
				...
				description: "Purchase created concurrently",
			};
			req.body = createData;
			...
			const promises = Array(5)
				.fill(null)
				.map(() => purchaseController.create(req as Request, res, next));
...
describe("Data Grouping Helper", () => {
	const testData = [
		{ id: 1, name: "Purchase 1", type: "email", category: "marketing" },
		{ id: 2, name: "Purchase 2", type: "sms", category: "notification" },
		{ id: 3, name: "Purchase 3", type: "email", category: "marketing" },
		{ id: 4, name: "Purchase 4", type: null, category: "general" },
		{ id: 5, name: "Purchase 5", type: "push", category: "notification" },
	];
	...
	describe("groupDataByField()", () => {
		...
		it("should handle undefined values by placing them in unassigned group", () => {
			const dataWithUndefined = [
				{ id: 1, name: "Purchase 1", type: "email" },
				{ id: 2, name: "Purchase 2" }, // missing type field
			];
		...
		it("should group by string values correctly", () => {
			...
			expect(result).to.have.property("Purchase 1");
			expect(result).to.have.property("Purchase 2");
			expect(result).to.have.property("Purchase 3");
			expect(result).to.have.property("Purchase 4");
			expect(result).to.have.property("Purchase 5");
			expect(result["Purchase 1"]).to.have.length(1);
		});
	});
});
