...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific purchase by its unique identifier with optional field selection
	 *     tags: [Purchase]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 */
	// Cache individual purchase with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:purchase:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve purchases with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [Purchase]
	 *     security:
	 *       - bearerAuth: []
	 *     parameters:
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache purchase list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:purchase:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
