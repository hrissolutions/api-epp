...
export const controller = (prisma: PrismaClient) => {
	const create = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			logActivity(req, {
				...
				description: `Approval level ${approvalLevel.level} created for workflow: ${approvalLevel.workflowId}`,
				page: {
					url: req.originalUrl,
			...
			try {
				...
				await invalidateCache.byPattern(`cache:approvalLevel:byWorkflowId:${approvalLevel.workflowId}:*`);
	...
	const getAll = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			const searchFields = ["workflowId", "role", "description"];
			if (query) {
	...
	const update = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			try {
				...
				await invalidateCache.byPattern("cache:approvalLevel:list:*");
				await invalidateCache.byPattern(`cache:approvalLevel:byWorkflowId:${updatedApprovalLevel.workflowId}:*`);
	...
	const remove = async (req: Request, res: Response, _next: NextFunction) => {
		...
		try {
			...
			try {
				await invalidateCache.byPattern(`cache:approvalLevel:byId:${id}:*`);
				await invalidateCache.byPattern("cache:approvalLevel:list:*");
				await invalidateCache.byPattern(`cache:approvalLevel:byWorkflowId:${existingApprovalLevel.workflowId}:*`);
