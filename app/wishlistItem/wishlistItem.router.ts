...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific wishlistItem by its unique identifier with optional field selection
	 *     tags: [WishlistItem]
	 *     parameters:
	 *       - in: path
	 *         name: id
	 ...
	 */
	// Cache individual wishlistItem with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:wishlistItem:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve wishlistItems with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [WishlistItem]
	 *     parameters:
	 *       - in: query
	 *         name: page
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache wishlistItem list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:wishlistItem:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
