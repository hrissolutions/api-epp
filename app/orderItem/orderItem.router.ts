...
export const router = (route: Router, controller: IController): Router => {
	 ...
	 *     description: Retrieve a specific orderItem by its unique identifier with optional field selection
	 *     tags: [OrderItem]
	 *     parameters:
	 *       - in: path
	 *         name: id
	 ...
	 */
	// Cache individual orderItem with predictable key for invalidation
	routes.get(
		"/:id",
		cache({
			ttl: 90,
			keyGenerator: (req: Request) => {
				...
				return `cache:orderItem:byId:${req.params.id}:${fields}`;
			},
		}),
		controller.getById,
	);
	 ...
	 *     description: Retrieve orderItems with advanced filtering, pagination, sorting, field selection, and optional grouping
	 *     tags: [OrderItem]
	 *     parameters:
	 *       - in: query
	 *         name: page
	 ...
	 *         $ref: '#/components/responses/InternalServerError'
	 */
	// Cache orderItem list with predictable key for invalidation
	routes.get(
		"/",
		cache({
			ttl: 60,
			keyGenerator: (req: Request) => {
				...
				return `cache:orderItem:list:${queryKey}`;
			},
		}),
		controller.getAll,
	);
